...
def get_optimal_concurrency():
    ...
    # 保守估计，留出系统资源
    return max(2, min(cpu_count - 1, 5))
...
def parse_arguments():
    ...
    parser.add_argument('--max-concurrent', type=int, default=get_optimal_concurrency(), help='Maximum number of concurrent queries (default: dynamic)')
...
def get_cached_result(cache_key, expiry=CACHE_EXPIRY):
    """获取缓存结果"""
    if cache_key in _query_cache:
        timestamp, data = _query_cache[cache_key]
        if (datetime.now() - timestamp).total_seconds() < expiry:
            return data
    return None

def set_cache_result(cache_key, data):
    """设置缓存结果"""
    _query_cache[cache_key] = (datetime.now(), data)
    return data

def cached_query(func):
    """缓存装饰器函数"""
    def wrapper(*args, **kwargs):
        # 如果参数中有 no_cache=True，则不使用缓存
        ...
        if cached_result is not None:
            print(f"[缓存] 使用缓存结果: {func.__name__}")
            return cached_result
            
        result = func(*args, **kwargs)
        return set_cache_result(cache_key, result)
    return wrapper
